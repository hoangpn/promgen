...
def save_host(sender, instance, **kwargs):
    ...
    if getattr(instance.farm, "project", None) and instance.farm.project.exporter_set:
...
def delete_host(sender, instance, **kwargs):
    """Only trigger write if parent project also has exporters"""
    if getattr(instance.farm, "project", None) is not None and instance.farm.project.exporter_set:
        trigger_write_config.send(instance)
